...
- T016 public fact, 2026-07-08: reverb is now a fixed mixer effect bus (`EffectBuses`/`ReverbEffectBus`) controlled through `Engine.setEffectBus` and measured through `Engine.effectBusSnapshot`; voice `reverb_send` remains the compatibility send into that bus.
- T017 public fact, 2026-07-08: `tools/run_validation.ps1` is the repeatable CPU/offline validation entry point; GPU validation is explicit with `-Gpu` and must fail if the requested dong build environment is missing.
- T018 public fact, 2026-07-08: GPU acoustic spike scene coverage is expanded to seven scenes (`open_air`, `thick_wall`, `wall_hole`, `door_closed`, `door_open`, `cave`, `open_field`) through real `in-dreaming/gpu` dispatch/readback; cave coverage is openness subset only, not GPU reverb/reflection.
