...
- 2026-07-08：新增 `tools/gpu_acoustic_spike`，包含 Bugu-owned CMake target、Slang compute shader 和 C runner。执行 `cmake -S tools/gpu_acoustic_spike -B build/gpu_acoustic_spike -G Ninja -DCMAKE_BUILD_TYPE=Release`、`cmake --build build/gpu_acoustic_spike --target bugu_gpu_acoustic_spike`、`build/gpu_acoustic_spike/bugu_gpu_acoustic_spike.exe`。该 runner 真实创建 GPU device、compile shader、create compute pipeline、dispatch `gpuComputeBindingDispatch`、wait queue、`gpuDownloadFromBuffer` readback，并验证 open_air/thick_wall/wall_hole/open_field against T010 CPU tolerances。
- 2026-07-08：GPU spike 输出保存到 [gpu-acoustic-spike-report.txt](../validation/gpu-acoustic-spike-report.txt)。结果：dedicated compute queue；open_air direct=0.51020；thick_wall transmission=0.01442/lowpass=1380；wall_hole portal=0.21704 direction=[0.894,0.447]；open_field openness=1.0；validation PASSED scenes=4。T013 进入 DONE。
